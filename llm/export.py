...
def load_hf_model(model_path):
    ...
    config.n_kv_heads = getattr(hf_model.config, 'num_key_value_heads', hf_model.config.num_attention_heads)
    ...
    for layer in model.layers:
        ...
        kv_dim = (config.dim // config.n_heads) * config.n_kv_heads  # GQA: k/v are smaller
        layer.attention.wq.weight = nn.Parameter(permute_reverse(hf_dict[f'model.layers.{i}.self_attn.q_proj.weight']))
        layer.attention.wk.weight = nn.Parameter(permute_reverse(hf_dict[f'model.layers.{i}.self_attn.k_proj.weight'], config.n_kv_heads, kv_dim, config.dim))
